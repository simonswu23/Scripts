...
################################################################################
  ...
  def pbDragonDartTargetting(attacker) # all of this just to get dragon darts targeting right, i hate it here
    ...
    if (darttype = :ELECTRIC || fieldsecondtype.include?(:ELECTRIC)) && (([:MOTORDRIVE,:VOLTABSORB,:LIGHTNINGROD, :STORMHEAL].include?(opp2.ability) && !opp2.moldbroken) || (Rejuv && @battle.FE == :GLITCH && opp2.species == :GENESECT && opp2.hasWorkingItem(:SHOCKDRIVE)))
      return [opp1]
    end
    if (darttype = :ELECTRIC || fieldsecondtype.include?(:ELECTRIC)) && (([:MOTORDRIVE,:VOLTABSORB,:LIGHTNINGROD, :STORMHEAL].include?(opp1.ability) && !opp1.moldbroken) || (Rejuv && @battle.FE == :GLITCH && opp1.species == :GENESECT && opp1.hasWorkingItem(:SHOCKDRIVE)))
      return [opp2]
    end
...
=end
  ...
  def smartDamageCategory(attacker,opponent)
    ...
    calcspatkmult *= 1.15 if attacker.pbPartner.ability == :PLUS ||attacker.ability == :PLUS
    calcspatkmult *= 1.15 if attacker.pbPartner.crested == :PLUSLE ||attacker.crested == :PLUSLE
    ...
    calcspdefmult*=0.85 if attacker.pbPartner.ability == :MINUS || attacker.ability == :MINUS
    calcspdefmult*=0.85 if attacker.pbPartner.crested == :MINUN || attacker.crested == :MINUN
...
################################################################################
  def pbTypeModifier(type,attacker,opponent,zorovar=false)
    ...
    if @battle.FE == :BEWITCHED
      ...
      mod1=4 if (otype1 == :STEEL) && (atype == :GRASS)
      mod2=4 if (otype2 == :STEEL) && (atype == :GRASS)
    ...
    if !opponent.moldbroken
      if (atype == :FIRE && opponent.ability == :FLASHFIRE && @battle.FE != :FROZENDIMENSION) || 
        ...
        (atype == :ELECTRIC && (opponent.ability == :VOLTABSORB || opponent.ability == :STORMHEAL || opponent.ability == :LIGHTNINGROD || opponent.ability == :MOTORDRIVE)) ||
        (atype == :GROUND && (opponent.ability == :LEVITATE || opponent.ability == :SOLARIDOL || opponent.ability == :LUNARIDOL || opponent.ability == :GRAVFLUX) && 
                              @battle.FE != :CAVE && @move != :THOUSANDARROWS && @move != :SOUTHERNWIND && opponent.isAirborne? && !(attacker.crested == :SANDACONDA && @battle.pbWeather == :SAND)) ||
        (atype == :GROUND && (opponent.ability == :EARTHEATER))
        mod1=0
      end
    end
    if @battle.FE == :CAVE || @move == :THOUSANDARROWS || @move == :SOUTHERNWIND || (attacker.crested == :SANDACONDA && @battle.pbWeather == :SAND)
      mod1=2 if (otype1 == :FLYING) && (atype == :GROUND)
      mod2=2 if (otype2 == :FLYING) && (atype == :GROUND)
    end
  ...
  def pbStatusMoveAbsorption(type,attacker,opponent)
    ...
    if (!(opponent.moldbroken) && (((opponent.ability == :DRYSKIN || opponent.ability == :WATERABSORB) &&  type == :WATER) || 
      ...
      if opponent.effects[:HealBlock]==0
        ...
        if (Rejuv && @battle.FE == :SHORTCIRCUIT) && (opponent.ability == :VOLTABSORB || opponent.ability == :STORMHEAL)
          damageroll = @battle.field.getRoll(maximize_roll: (@battle.state.effects[:ELECTERRAIN] > 0))
  ...
  def pbTypeModMessages(type,attacker,opponent)
    ...
    if ((opponent.ability == :DRYSKIN && !(opponent.moldbroken)) && (type == :WATER || (!secondtype.nil? && secondtype.include?(:WATER)))) ||
      ...
      if opponent.effects[:HealBlock]==0
        negator = getAbilityName(opponent.ability)
        if ![:WATERABSORB,:VOLTABSORB,:STORMHEAL,:DRYSKIN,:EARTHEATER].include?(opponent.ability)
          negator = getItemName(opponent.item) if (Rejuv && @battle.FE == :GLITCH && opponent.species == :GENESECT && (opponent.item == :DOUSEDRIVE || opponent.item == :CHILLDRIVE))
          negator = "unquenchable thirst" if (Rejuv && @battle.FE == :DESERT) && (opponent.hasType?(:GRASS) || opponent.hasType?(:WATER)) && @battle.pbWeather == :SUNNYDAY
        end
        if (Rejuv && @battle.FE == :SHORTCIRCUIT) && (opponent.ability == :VOLTABSORB || opponent.ability == :STORMHEAL)
    ...
    # Immunity abilities
    if (type == :FIRE || (!secondtype.nil? && secondtype.include?(:FIRE))) && (opponent.ability == :HEATPROOF || (opponent.ability == :MAGMAARMOR && (@battle.FE == :DRAGONSDEN || @battle.FE == :VOLCANICTOP || @battle.FE == :INFERNAL) && !(opponent.moldbroken)))
      @battle.pbDisplay(_INTL("{1}'s {2} made {3} ineffective!",
        opponent.pbThis,getAbilityName(opponent.ability),self.name))
      return 0
    end
    if (type == :POISON || (!secondtype.nil? && secondtype.include?(:POISON))) && (opponent.ability == :PASTELVEIL && !opponent.moldbroken)
      @battle.pbDisplay(_INTL("{1}'s {2} made {3} ineffective!",
        opponent.pbThis,getAbilityName(opponent.ability),self.name))
      return 0
    end
    if (type == :POISON || (!secondtype.nil? && secondtype.include?(:POISON))) && (opponent.pbPartner.ability == :PASTELVEIL && !opponent.pbPartner.moldbroken)
      @battle.pbDisplay(_INTL("{1}'s {2} made {3} ineffective!",
        opponent.pbPartner.pbThis,getAbilityName(opponent.pbPartner.ability),self.name))
      return 0
    end
    ...
    if opponent.crested
      ...
      when :BRAVIARY
        typemod /= 2 if [:ROCK, :DARK, :BUG].include?(type)
      when :GOODRA
    ...
    if @move == :SOUTHERNWIND
      if @battle.FE == :SKY
        if ((PBTypes.oneTypeEff(:GROUND, opponent.type1) > 2) || (PBTypes.oneTypeEff(:GROUND, opponent.type1) < 2 && $game_switches[:Inversemode]))
          typemod*=2
        end
        if ((PBTypes.oneTypeEff(:GROUND, opponent.type2) > 2) || (PBTypes.oneTypeEff(:GROUND, opponent.type2) < 2 && $game_switches[:Inversemode]))
          typemod*=2
        end
      else
        typemod2=pbTypeModifier(:GROUND,attacker,opponent)
        typemod3= ((typemod*typemod2)/4)
        typemod=typemod3
      end
    end
  ...
  def getSecondaryType(attacker)
    ...
    secondtype.push(:GROUND) if @move == :SOUTHERNWIND
...
################################################################################
  def pbAccuracyCheck(attacker,opponent)
    ...
    if attacker.ability == :ILLUMINATE
      accuracy*=1.15
    end
    ...
    if partner && partner.ability == :ILLUMINATE
      accuracy*=1.15
    end
...
################################################################################
  def pbCritRate?(attacker,opponent)
    ...
    return 3 if @move == :ONEBLOW
    return 3 if @move == :AEROBLAST && @battle.FE == :SKY
  ...
  def pbCalcDamage(attacker,opponent,options=0, hitnum: 0)
    @category = :physical if attacker.crested == :RILLABOOM && isSoundBased?
    ...
    case attacker.ability
      ...
      when :ACCELERATE    then basemult*=1.5 if @priority > 0
      when :IRONFIST
        if @battle.FE == :CROWD
      ...
      when :RIVALRY       then basemult*=1.3 if opponent.hasType?(attacker.type1) || (!attacker.type2.nil? && opponent.hasType?(attacker.type2))
    ...
    case attacker.crested
      ...
      # when :WIGGLYTUFF then basemult *= 1.5 if isSoundBased?
      when :SAWSBUCK  
        case attacker.form
    ...
    case @battle.FE
      ...
      when :DEEPEARTH
        if (priorityCheck(attacker, opponent) > 0) && @basedamage > 0 && @move != :CHIRP
          provimult=0.7
        ...
        if (priorityCheck(attacker, opponent) < 0) && @basedamage > 0
          provimult=1.3
    ...
    case attacker.ability
      ...
      when :PROTOSYNTHESIS then atkmult*=1.3 if (attacker.effects[:Quarkdrive][0] == PBStats::ATTACK && pbIsPhysical?(type)) || (attacker.effects[:Quarkdrive][0] == PBStats::SPATK && pbIsSpecial?(type))
    end
    ...
    if (attacker.ability == :PLUS || attacker.pbPartner.ability == (:PLUS)) && pbIsSpecial?(type)
      atkmult*=1.15
      if (attacker.crested == :PLUSLE || attacker.pbPartner.crested == (:PLUSLE)) && pbIsSpecial?(type)
        atkmult*=1.15
      end
    end
    if @battle.FE == :FAIRYTALE
    ...
    if (attacker.ability == :MINUS || attacker.pbPartner.ability == (:MINUS)) && pbIsSpecial?(type)
      defmult*=0.85
      if (attacker.crested == :MINUN || attacker.pbPartner.crested == :MINUN) && pbIsSpecial?(type)
        defmult*=0.85
      end
    end
    ...
    if opponent.hasWorkingItem(:METALPOWDER) && (opponent.pokemon.species == :DITTO) && pbIsPhysical?(type)
      defmult*=2.0
    end

    defmult*=1.5 if opponent.crested == :RILLABOOM && pbIsSpecial?(type) && (@battle.FE == :GRASSY || @battle.FE == :FOREST || @battle.state.effects[:GRASSY] > 0)
    ...
    case type
      when :FIRE
        ...
        damage*=0.5 if (@battle.weather == :RAINDANCE || @battle.state.effects[:AbsoluteZero]) && !attacker.hasWorkingItem(:UTILITYUMBRELLA)
        damage*=0.5 if opponent.ability == :WATERBUBBLE
      when :WATER
        damage*=1.5 if @battle.weather == :RAINDANCE && !opponent.hasWorkingItem(:UTILITYUMBRELLA)
        damage*=0.5 if (@battle.weather == :SUNNYDAY || @battle.state.effects[:DesertNova]) && !attacker.hasWorkingItem(:UTILITYUMBRELLA)
      ...
      when :ICE
        ...
        damage*=1.5 if @battle.state.effects[:AbsoluteZero] && !opponent.hasWorkingItem(:UTILITYUMBRELLA)
      when :ROCK 
        damage*=1.5 if @battle.state.effects[:DesertNova] && !opponent.hasWorkingItem(:UTILITYUMBRELLA)
    end
    # Critical hits
    ...
    case attacker.crested
      ...
      when :SEVIPER then typecrest = true if type == :DARK
      when :ZOROARK
    ...
    if (opponent.anticipation && opponent.damagestate.typemod>4)
      finalmult*=0.5
      @battle.pbDisplay(_INTL("{1} braced itself for the attack!",opponent.pbThis))
      opponent.anticipation = false
    end
     ...
     @move!=:SPECTRALTHIEF &&  @move!=:HYPERSPACEHOLE &&  @move!=:HYPERSPACEFURY #spectral thief/ hyperspace hole/ hyperspace fury
      ...
      if damage>=opponent.hp
        ...
        if @function==0xE9 # False Swipe
          ...
          damage=damage-1
        elsif (opponent.ability == :STALWART && opponent.moldbroken && @battle.FE == :COLOSSEUM && !opponent.pokemon.rampCrestUsed)
          opponent.damagestate.rampcrestused=true
          opponent.pokemon.rampCrestUsed = true
          damage=damage-1
        end
...
################################################################################
  def pbEffectMessages(attacker,opponent,ignoretype=false)
    ...
    elsif opponent.damagestate.rampcrestused && opponent.crested == :RAMPARDOS
      @battle.pbDisplay(_INTL("{1} hung on using its Rampardos Crest!",opponent.pbThis))
      opponent.damagestate.rampcrestused=false
    elsif opponent.damagestate.rampcrestused # Stalwart branch
  ...
  def priorityCheck(attacker, opponent=nil)
    pri = self.priority
    ...
    pri += 1 if (attacker.ability == :GALEWINGS || attacker.crested == :BRAVIARY) && @type==:FLYING
    pri += 1 if (attacker.ability == :HIVEQUEEN || attacker.pbPartner.ability == :HIVEQUEEN) && @type==:BUG 

    # forewarn
    pri += 1 if (opponent && !opponent.hasMovedThisRound? && attacker.forewarn.include?(opponent.selectedMove))
