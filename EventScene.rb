...
class PictureSprite < SpriteWrapper
  def initialize(viewport, picture, z = nil)
    super(viewport)
    @picture = picture
    @pictureBitmap = nil
    @customBitmap = nil
...
class EventScene
  ...
  def addGif(x,y,filename) 
    ...
    @pictures[num]=picture
    @picturesprites[num]=PictureSprite.new(@viewport,picture)
    @picturesprites[num].setCustomBitmap(Bitmap.new(filename))
    @picturesprites[num].bitmap.play
    return picture
  end
  ...
  def addImage(x,y,name,z=nil)
    num=@pictures.length
    picture=PictureEx.new(num)
    picture.name=name
    picture.moveXY(0,0,x,y)
    @pictures[num]=picture
    @picturesprites[num]=PictureSprite.new(@viewport,picture,z)
    return picture
  end
