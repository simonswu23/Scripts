...
class PokeBattle_Move_1100 < PokeBattle_Move
  ...
  def pbAdditionalEffect(attacker,opponent)
    ...
    volc = true
    irocks = true
    while (!chosen && (spikes || tspikes || webs || rocks || steel || volc || irocks))
      hazard = @battle.pbRandom(7)
      case hazard
        when 0
        ...
        when 2
          if (!attacker.pbOpposingSide.effects[:StealthRock])
            ...
            if !@battle.pbIsOpposing?(attacker.index)
              @battle.pbDisplay(_INTL("The pointed stones float around your opponent's team!"))
            else
              @battle.pbDisplay(_INTL("The pointed stones float from around your team!"))
            end
            chosen = true
          else
            rocks = false
        ...
        when 6
          if (!attacker.pbOpposingSide.effects[:InvStealthRock])
            # pbShowAnimation(:STEALTHROCK,attacker,opponent)
            attacker.pbOpposingSide.effects[:InvStealthRock]=true
            if !@battle.pbIsOpposing?(attacker.index)
              @battle.pbDisplay(_INTL("The mysterious stones float around your opponent's team!"))
            else
              @battle.pbDisplay(_INTL("The mysterious stones float around your team!"))
            end
            chosen = true
          else
            rocks = false
